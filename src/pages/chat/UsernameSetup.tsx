...
export default function UsernameSetup() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("profiles").select("username, avatar_url, completed").eq("user_id", user.id).maybeSingle().then(({ data }) => {
  ...
  const save = async () => {
    if (!user) return;
    if (!USERNAME_RE.test(username)) {
      toast.error("اسم المستخدم: حروف إنجليزية صغيرة وأرقام و _ ، 3 إلى 20 حرف");
      return;
    }
    if (available === false) {
    ...
    try {
      const { error } = await supabase
        .from("profiles")
        .update({ username, avatar_url: avatarPath })
        .eq("user_id", user.id);
      if (error) throw error;
      toast.success("تم!");
      ...
      <div className="card-horror p-6 w-full max-w-md">
        ...
        <div className="mb-6">
          ...
          <p className="text-xs mt-1 text-muted-foreground">
            {!username ? "حروف إنجليزية صغيرة وأرقام و _ ، 3-20 حرف"
              : !USERNAME_RE.test(username) ? "تنسيق غير صحيح"
