...
/**
 * يبدأ عملية دفع كاشير لمنتج معيّن:
 *  1) يطلب رابط الدفع من Edge Function (create-kashier-payment) — الرابط
 *     بيتحسب بالكامل على السيرفر عشان مفتاح كاشير السري ميبانش أبدًا
 *     في كود الموبايل.
 *  2) يفتحه في متصفح داخل التطبيق (Capacitor Browser) بدل ما يودّي
 *     المستخدم برّه التطبيق تمامًا.
 *  3) لما كاشير يرجّع المستخدم لرابط com.rebh.app://payment-result
 *     بعد الدفع، بنقفل المتصفح الداخلي تلقائيًا ونرجع للتطبيق —
 *     الفتح الفعلي للميزة بيحصل من السيرفر (كاشير-ويبهوك) مش من هنا،
 *     فبنكتفي هنا بعمل refresh لحالة المشتريات.
 */
export async function startKashierCheckout(
  ...
  if (error || !data?.paymentUrl) {
    console.error("[Kashier] create-kashier-payment failed", error, data);
